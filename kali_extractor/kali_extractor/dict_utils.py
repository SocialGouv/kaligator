...
def xpath_to_keys(selector):
    """
        ...
        e.g. xpath_to_keys('root/subdocs[2]/key1')
             => ['root', 'subdocs', 2, 'key1']
    """
...
def deep_set(dictionary, xpath_selector, value):
    """
        ...
        e.g. deep_set({'a': [{'b': 10}, {'b': 20}]}, 'a[1]/b', 30)
             will set the b value in the second item
    """
...
def deep_get(dictionary, xpath_selector):
    """
        yields nested values from a dictionary with the associated selector.
        e.g. deep_get({'a': {'b': 10}}, 'a/b']) returns (10, 'a/b'])
        e.g. deep_get({'a': [{'b': 10}, {'b': 20}]}, 'a/b')
             yields (10, 'a[0]/b') and then (20, 'a[1]/b'])
    """
    keys = xpath_to_keys(xpath_selector)
    ...
    if current_dict is not None:
        ...
        if len(leftover_keys) == 0:
            ...
            for idx, child in enumerate(value):
                yield from recursive_get(
                    child, leftover_keys,
                    "%s/%s[%s]" % (built_selector, key, idx)
                )
        else:
            yield from recursive_get(
                value, leftover_keys, "%s/%s" % (built_selector, key)
            )
